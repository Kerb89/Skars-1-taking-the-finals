...
## API Arte — Art Institute of Chicago (AIC)

- **URL base**: `https://api.artic.edu/api/v1/artworks`
- **Auth**: Nessuna (completamente aperta)
- **Licenza**: CC0 per le opere public domain
- **Ricerca**: `/search?q={query}&query[term][is_public_domain]=true&fields=id,title,artist_display,date_display,image_id`
- **Immagine via IIIF**: `https://www.artic.edu/iiif/2/{image_id}/full/843,/0/default.jpg`
  - La larghezza (843) è configurabile nell'URL: usa 500 per il base64 del quiz
- **Campi utili**: `title`, `artist_display`, `date_display`, `image_id`, `is_public_domain`
- **Vantaggio**: titolo e artista arrivano dalla catalogazione ufficiale del museo — nessun rischio di titoli sbagliati

### Workflow AIC
1. Cerca: `https://api.artic.edu/api/v1/artworks/search?q=vermeer&query[term][is_public_domain]=true&fields=id,title,artist_display,image_id`
2. Prendi `image_id` dal risultato
3. Scarica: `https://www.artic.edu/iiif/2/{image_id}/full/500,/0/default.jpg`
4. Salva in `art_questions_images/da_usare/`, converti in base64

---

## API Arte — Cleveland Museum of Art

- **URL base**: `https://openaccess-api.clevelandart.org/api/artworks/`
- **Auth**: Nessuna
- **Licenza**: CC0 (oltre 61.000 opere)
- **Ricerca**: `?q={query}&cc0=1` oppure `?artists={nome}&cc0=1`
- **Campi utili**: `title`, `creators[].description`, `creation_date`, `images.web.url`
- **Download diretto**: il campo `images.web.url` è un URL immagine scaricabile con curl
- **Vantaggio**: complementare al Met per europei classici, URL immagine diretto nella risposta

### Workflow Cleveland
1. Cerca: `https://openaccess-api.clevelandart.org/api/artworks/?q=monet+water+lilies&cc0=1`
2. Prendi `images.web.url` dal risultato
3. Scarica con curl, salva, converti in base64
...
## Policy User-Agent (obbligatoria)

Tutte le richieste HTTP verso API esterne DEVONO usare un User-Agent identificato:

```
QuizzoneBot/1.0 (progetto quiz privato; contatto: kerberozzo89@gmail.com)
```

### In Python (requests)
```python
HEADERS = {"User-Agent": "QuizzoneBot/1.0 (progetto quiz privato; contatto: kerberozzo89@gmail.com)"}
requests.get(url, headers=HEADERS)
```

### In curl
```bash
curl.exe -A "QuizzoneBot/1.0 (contatto: kerberozzo89@gmail.com)" -L -o file.jpg {url}
```

### Dove è critico
- **Wikimedia** (Commons, Wikidata SPARQL, Wikipedia): UA generico = 403 garantito.
- **MusicBrainz**: UA richiesto esplicitamente nella documentazione.
- **Met, AIC, Cleveland**: non bloccano, ma buona pratica identificarsi comunque.

### Regola anti-mascheramento
**MAI scavalcare un 403 fingendosi un browser** (copiando lo UA di Chrome/Firefox).
La policy Wikimedia è esplicita: comportamento da bot con UA da browser viene
classificato come malevolo ed è il modo per farsi bloccare l'IP.
Se un 403 persiste con UA conforme, è un rate limit: si aspetta, non si maschera.
