...
# --- pattern di "arricchimento formale" che regalano la risposta ------------
# Ogni voce: (nome, regex). Se un pattern compare SOLO nella risposta
# corretta e in nessun distrattore, la domanda e' regalata -> ERRORE.
ENRICH_PATTERNS = [
...
# soglie per l'outlier di lunghezza della risposta corretta
LEN_RATIO_MAX = 1.5    # corretta > 1.5x della media dei distrattori
LEN_DIFF_MAX = 15      # ...oppure piu' lunga di +15 caratteri della media


def enrichment_tags(text: str):
    """Ritorna l'insieme dei nomi dei pattern di arricchimento presenti."""
    return {name for name, rx in ENRICH_PATTERNS if rx.search(text)}


def giveaway_checks(num, opts, sol, errors, warnings, label=""):
    """Controlli anti-regalo su una singola domanda.

    opts: dict lettera -> testo opzione (tutte e 4 presenti)
    sol:  set di lettere corrette
    Appende messaggi a errors/warnings in-place.
    """
    correct = [opts[L] for L in sorted(sol) if L in opts]
    distr = [opts[L] for L in "ABCD" if L not in sol]
    if not correct or not distr:
        return

    # 1. pattern presente in una corretta e in NESSUN distrattore -> ERRORE
    distr_tags = set()
    for d in distr:
        distr_tags |= enrichment_tags(d)
    for ct in correct:
        for tag in enrichment_tags(ct) - distr_tags:
            errors.append(
                f"D{num}{label}: regalo formale — '{tag}' presente solo "
                f"nella risposta corretta e in nessun distrattore.")

    # 2. reverse-tell: pattern presente in UN solo distrattore e in nessuna
    #    corretta (lo marca come sbagliato per esclusione) -> WARNING
    corr_tags = set()
    for ct in correct:
        corr_tags |= enrichment_tags(ct)
    tag_count = Counter()
    for d in distr:
        for tag in enrichment_tags(d):
            tag_count[tag] += 1
    for tag, n in tag_count.items():
        if n == 1 and tag not in corr_tags:
            warnings.append(
                f"D{num}{label}: '{tag}' presente in un solo distrattore; "
                f"possibile tell inverso, uniformare.")

    # 3. outlier di lunghezza: corretta molto piu' lunga dei distrattori
    avg_d = sum(len(d) for d in distr) / len(distr)
    for ct in correct:
        if (len(ct) > max(len(d) for d in distr)
                and avg_d > 0
                and (len(ct) / avg_d >= LEN_RATIO_MAX
                     or len(ct) - avg_d >= LEN_DIFF_MAX)):
            errors.append(
                f"D{num}{label}: regalo formale — risposta corretta di "
                f"{len(ct)} caratteri contro una media distrattori di "
                f"{avg_d:.0f}; accorciare o allungare i distrattori.")
...
def check(text: str):
    ...
    for num in sorted(questions):
        ...
        # --- 4. uniformita' opzioni + anti-regalo (bloccante) ---
        if not missing:
            ...
            giveaway_checks(num, opts, sol, errors, warnings)
